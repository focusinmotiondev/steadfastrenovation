...
export default function AdminShell({ title, subtitle, children, actions }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F4F1EB" }}>
      <aside style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 250, background: C.bgDark, color: "#fff", padding: "24px 18px", display: "flex", flexDirection: "column", zIndex: 100, overflowY: "auto" }}>
        ...
        <nav style={{ display: "grid", gap: 4, flex: 1 }}>
          {links.map((link) => {
            ...
            return <Link key={link.href} href={link.href} style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: 10, color: active ? "#fff" : "rgba(255,255,255,0.6)", background: active ? C.accent : "transparent", padding: "11px 14px", borderRadius: 10, fontFamily: "var(--font-body)", fontSize: 14, fontWeight: active ? 600 : 400 }}>{link.label}</Link>;
          })}
        </nav>
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: 16, marginTop: 16 }}>
          <Link href="/" style={{ display: "block", textDecoration: "none", color: "rgba(255,255,255,0.5)", fontFamily: "var(--font-body)", fontSize: 13, padding: "10px 14px", borderRadius: 10, marginBottom: 6 }}>View Website</Link>
          <button onClick={() => { logoutAdmin(); router.push("/admin/login"); }} style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "rgba(255,255,255,0.5)", fontFamily: "var(--font-body)", fontSize: 13, cursor: "pointer", textAlign: "left" }}>Sign Out</button>
        </div>
      </aside>
