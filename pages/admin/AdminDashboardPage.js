...
import { currency, formatDate, getInvoices, getLeads, getClientBalance, saveLead, STATUS_COLORS, SERVICE_OPTIONS, AREA_OPTIONS, BUDGET_OPTIONS, TIMELINE_OPTIONS } from "../../lib/crm";
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setLeads(l); setInvoices(i); setLoading(false);
    }
    load();
  }, []);
  ...
  const handleAddClient = async () => {
    ...
    await saveLead({ ...addForm, howFound: "Admin — Manual Entry", preferredContact: ["Phone"] });
    const l = await getLeads();
    setLeads(l);
    ...
    <RequireAdmin>
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }} className="admin-stats">
          {stats.map(([label, value, color], i) => <div key={i} style={cardS}><p style={eyebrow}>{label}</p><p style={{ fontFamily: "var(--font-display)", fontSize: i >= 2 ? 22 : 32, fontWeight: 700, color }}>{value}</p></div>)}
        </div>
                ...
                <tbody>{active.slice(0, 15).map((l) => {
                  ...
                  return (
                    <tr key={l.id} style={{ borderBottom: `1px solid ${C.borderLight}` }}>
                      ...
                      <td style={tdS}>{bal.totalInvoiced > 0 ? <span style={{ fontWeight: 700, color: bal.paid ? "#22A06B" : "#DE350B", fontSize: 13 }}>{bal.paid ? "PAID" : currency(bal.balance)}</span> : <span style={{ color: C.textLight, fontSize: 12 }}>—</span>}</td>
