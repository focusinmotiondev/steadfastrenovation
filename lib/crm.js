...
export const getNextInvoiceNumber = () => { const n = readJson(CRM_NEXT_INV_KEY, 1350); writeJson(CRM_NEXT_INV_KEY, n + 1); return n; };
export const getInvoices = () => readJson(CRM_INVOICE_KEY, []).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
export const saveInvoice = (payload) => { const invoices = getInvoices(); const invoice = { id: `inv_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, createdAt: new Date().toISOString(), status: "draft", invoiceNumber: getNextInvoiceNumber(), ...payload }; invoices.unshift(invoice); writeJson(CRM_INVOICE_KEY, invoices); const lead = getLeadById(payload.clientId); if (lead) { updateLead(payload.clientId, { status: lead.status === "new" ? "quoted" : lead.status, invoices: [...(lead.invoices || []), invoice.id] }); } return invoice; };
